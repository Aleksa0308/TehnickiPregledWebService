...
const SingleEvidencija = () => {
  ...
  const getPostojeci = async () => {
    const response = await fetch(
      `http://localhost:8080/api/evidencije/${day}-${month}-${year}`,
      requestOptions
    );
  ...
  const getTotal = async () => {
    const response = await fetch(
      "http://localhost:8080/api/evidencije/total",
      requestOptions
    );
    const data = await response.json();
    setTotal(data);
  ...
  const getPrevious = async () => {
    const response = await fetch(
      `http://localhost:8080/api/evidencije/previous/${year}-${month}-${day}`,
      requestOptions
    );
  ...
  const changeDate = (e) => {
    ...
    window.location.replace(
      `http://localhost:3000/evidencija/${tmp[2]}-${tmp[1]}-${tmp[0]}`
    );
  };
  ...
  return (
    <div className="flex flex-col items-center  xl:px-12 lg:px-2 ">
      ...
      <div>
        ...
        {specifikacija.map((spec) => {
          if (postojeci.filter((p) => p.num === spec.num).length !== 0) {
            return <></>;
          } else {
            return (
              <Zapis
