...
const SingleStatistics = () => {
  ...
  const getTehnicki = async () => {
    const response = await fetch(
      Adresa.adresa + `:8080/api/statistika/tehnicki/${datumFull}`,
      requestOptions
    );
  ...
  const getPlaceno = async () => {
    const response = await fetch(
      Adresa.adresa + `:8080/api/statistika/placeno/${datumFull}`,
      requestOptions
    );
    const data = await response.json();
  ...
  const getTotal = async () => {
    const response = await fetch(
      Adresa.adresa + `:8080/api/statistika/total/${datumFull}`,
      requestOptions
    );
    const data = await response.json();
    setTotal(data.total);
  ...
  const getData = async () => {
    const response = await fetch(
      Adresa.adresa + `:8080/api/statistika/grupisano/${datumFull}`,
      requestOptions
    );
  ...
  const setPageParams = (value) => {
    ...
    window.location.replace(
      Adresa.adresa + `:3000/statistika/${tmp1[2]}-${tmp1[1]}-${tmp1[0]}&${tmp2[2]}-${tmp2[1]}-${tmp2[0]}`
    );
  };
